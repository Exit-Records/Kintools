...
- **PWA Support**: Every new Kin tool *must* include Progressive Web Application (PWA) features, including `icon.svg`, `manifest.json`, and specific HTML meta tags for home screen installation. A JavaScript snippet generates a 180x180 PNG `apple-touch-icon` at runtime for iOS. **CRITICAL RULE:** If pasted code does not include the apple-touch-icon canvas script (and matching `<link rel="apple-touch-icon" id="apple-touch-icon" href="">` in `<head>`), add both automatically — never ship a new tool without it. The canvas script draws a 180×180 rounded rect using the tool's landing page gradient colors, then overlays the tool's icon SVG at 96×96 centered, and sets the link href to the resulting PNG data URL.
- **Tool Naming**: Short names for PWA installations consistently follow the `KIN-NNN` format (e.g., `KIN-015`).
- **Creator Name**: Always `Darren` unless explicitly specified otherwise by the user.
- **Default Theme**: All new Kin tools MUST default to **light mode**. Set `class="light"` on `<html>`, theme-color meta to the light value, and `dark = false` in JS with button starting as `🌙`. Never default to dark unless the spec explicitly requires it.
